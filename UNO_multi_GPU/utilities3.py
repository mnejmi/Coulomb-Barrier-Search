...
def data_prepare(cfg: TrainConfig, inputpath):
    ...
    index = cfg.stepsize * cfg.iteration
...
def normalizeData(numfields, a_train, u_train, a_val, u_val):
    ...
    for i in range(numfields):
        print(f'm_a[{i}]={m_a[i]:.5f}, std_a[{i}]={std_a[i]:.5f}')
        print(f'm_u[{i}]={m_u[i]:.5f}, std_u[{i}]={std_u[i]:.5f}')
    a_train -= m_a
    a_train /= std_a
    u_train -= m_u
    u_train /= std_u
...
def full_loop(rank, model, optimizer, scheduler, train_loader, valid_loader, cfg: TrainConfig):
    train_losses = []
    val_losses = []
    LR = []
