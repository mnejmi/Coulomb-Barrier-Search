...
class UNetNeuralOperator(nn.Module):
    ...
    def __init__(self, in_width: int, width: int, pad: int=0):
        super().__init__()
        ...
        self.G_in = FourierOperatorBlock2d(self.width, self.width, 56, 56, 26, 26)
        self.G_out = FourierOperatorBlock2d(2 * self.width, self.width, 56, 56, 26, 26)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_c12_cat = torch.cat([x_c12, x_in_56], dim=1)
        x_out_56 = self.G_out(x_c12_cat, D1, D2)
        if self.padding and self.padding > 0:
            pad = self.padding
            x_out_56 = x_out_56[..., :-pad, :-pad]
        x_out_56 = x_out_56.permute(0, 2, 3, 1)
        x_fc1 = F.gelu(self.fc1(x_out_56))
        x_out = self.fc2(x_fc1)
        return x_out

class ConvolutionalGRUCell(nn.Module):

    def __init__(self, input_size, hidden_size, kernel_size=3):
        super(ConvolutionalGRUCell, self).__init__()
    ...
    def forward(self, input_tensor, hidden_state):
        stacked_inputs = torch.cat([input_tensor, hidden_state], dim=1)
        update = torch.sigmoid(self.update_gate(stacked_inputs))
        reset = torch.sigmoid(self.reset_gate(stacked_inputs))
        out_inputs = torch.tanh(self.out_gate(torch.cat([input_tensor, hidden_state * reset], dim=1)))
        new_state = hidden_state * (1 - update) + out_inputs * update
        return new_state

class RecurrentNeuralOperator(nn.Module):

    def __init__(self, in_width: int, width: int, pad: int=0):
        super().__init__()
        self.uno = UNetNeuralOperator(in_width=in_width, width=width, pad=pad)
        self.gru_cell = ConvolutionalGRUCell(input_size=6, hidden_size=6)

    def forward(self, x: torch.Tensor, h_prev: Optional[torch.Tensor]=None) -> tuple[torch.Tensor, torch.Tensor]:
        uno_out = self.uno(x)
        if h_prev is None:
            h_prev = torch.zeros_like(uno_out)
        uno_out_c = uno_out.permute(0, 3, 1, 2)
        h_prev_c = h_prev.permute(0, 3, 1, 2)
        h_next_c = self.gru_cell(uno_out_c, h_prev_c)
        h_next = h_next_c.permute(0, 2, 3, 1)
