...
energy_end = 60
step = 0.5
record = np.zeros((int((energy_end - energy_start) / step) + 1, 3))
b_min = 3
b_max = 4
for e in range(int((energy_end - energy_start) / step) + 1):
    ecm = energy_start + e * step

    def search_b(x):
        return predictions(ecm, x) - 1.5
    b_root = bisection_method(search_b, b_min, b_max)
    if b_root < 2:
        b_min = b_root
        b_max = b_root + 0.5
    elif b_root < 4:
        ...
        b_min = b_root - 0.1
        b_max = b_root + 0.3
    Lc = math.sqrt(2 * mu * ecm) * b_root
    fcs = math.pi * Lc ** 2 / (2 * mu * ecm)
    record[e, 0] = ecm
    record[e, 1] = fcs
    record[e, 2] = b_root
    print(f'energy = {ecm}, fusion cross section = {fcs}, b_root = {b_root}', flush=True)
file_path = 'FusionCrossSection_out2.txt'
np.savetxt(file_path, record, delimiter=',', fmt='%.10f')