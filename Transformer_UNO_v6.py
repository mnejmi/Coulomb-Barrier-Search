...
class Transformer_UNO_v6(nn.Module):
    def __init__(self, in_width: int, width: int, pad: int = 0, dropout_rate: float = 0.0):
        super().__init__()
        ...
        self.time_mlp = nn.Sequential(
            SinusoidalPositionEmbeddings(width * 4),
        ...
        # High-Frequency Shell Blocks
        self.G_in = OperatorBlock_2D(self.width, self.width, 56, 56, 26, 26)
        self.G_out = OperatorBlock_2D(2 * self.width, self.width, 56, 56, 26, 26)
    ...
    def forward(self, x: torch.Tensor, t_map: torch.Tensor = None) -> torch.Tensor:
        """
        x: [B, H, W, C]  (Initial State T=0)
        t_map: [B, H, W, 1] (Target Continuous Time Map for autograd) - optional for inference
        """
        B, H, W, _ = x.shape
        device = x.device
        dtype = x.dtype
        ...
        # 3. Time Injection at the Bottleneck (skip if t_map not provided)
        if t_map is not None:
            t_map_down = F.adaptive_avg_pool2d(t_map.permute(0, 3, 1, 2), (D1 // 16, D2 // 16))
            t_emb_spatial = self.time_mlp(t_map_down.permute(0, 2, 3, 1))
            t_emb_spatial = t_emb_spatial.permute(0, 3, 1, 2)
            x_c8 = x_c8 + t_emb_spatial
        
        # 4. Upsampling Decoders (Skip connections could be added here, but UNO natively relies on deep chains)
