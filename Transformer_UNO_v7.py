...
class Transformer_UNO_v7(nn.Module):
    def __init__(self, in_width: int, width: int, pad: int = 0, dropout_rate: float = 0.0):
        ...
        # Time Embedding MLP (Global Time Signal)
        ...
        # We set Normalize=False inside the block to avoid InstanceNorm destroying absolute physical magnitudes.
        # We will apply LayerNorm manually between the blocks.
    ...
    def forward(self, x: torch.Tensor, t_map: torch.Tensor = None) -> torch.Tensor:
        ...
        # 3. Time Embedding Extraction (skip if t_map not provided)
        use_time = t_map is not None
        if use_time:
            t_scalar = t_map[:, 0, 0, :] 
            t_emb = self.time_mlp(t_scalar) # [B, 16*width]
        ...
        # 5. Multi-Scale Time Injection in Decoder (skip if t_map not provided)
        if use_time:
