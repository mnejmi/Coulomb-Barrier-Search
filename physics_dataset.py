...
class UNOTIterableDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        # Partition the file list among all global workers
        local_files = [f for i, f in enumerate(self.file_list) if i % total_splits == global_worker_id]
        
        # Infinite loop to prevent StopIteration deadlocks in DDP
        while True:
            if self.shuffle:
            ...
            for file_name in local_files:
                ...
                for frame_idx in frame_indices:
                    ...
                    u = np.stack(u_frames, axis=0) # [iteration, H, W, C]
                    yield torch.as_tensor(a, dtype=torch.float32), torch.as_tensor(u, dtype=torch.float32)
                    
                # Free memory explicitly after yielding all frames for this trajectory
                del phis
...
def data_prepare(cfg, inputpath):
    ...
    for f in all_files:
        match = re.search(r'data_2d_(\d+)\.(pt|npy)', f)
        if match:
    ...
    # Optional debugging limit (user requested s_select = 3000)
    limit_data = getattr(cfg, 'limit_data', None)
    if limit_data is not None and limit_data > 0:
        # Number of base groups to select to reach limit_data files (assuming 8 symmetries per group)
    ...
    # Pass the lists directly to the IterableDataset instead of loading them
