...
def predictions_output(m, std, model, cfg: TrainConfig, a_initial=None, i=3, n_step: int=60):
    numfields = 6
    dx = 0.9
    grid = 56
    mn = 939.565
    ...
    if a_initial is None:
        ...
        with torch.no_grad():
            ...
            for t in range(1, n_step):
                ...
                predictions = out[0]
    predictions_output = phis_pred[:t + 1] * std + m
    num = fragments(predictions_output[-1, :, :, 0] + predictions_output[-1, :, :, 1], rho_cut=0.4)
