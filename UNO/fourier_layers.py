...
class SpectralConv1d_Uno(nn.Module):
    ...
    def forward(self, x, dim1=None):
        if dim1 is not None:
            self.dim1 = dim1
        batchsize = x.shape[0]
        x_ft = torch.fft.rfft(x.to(torch.float32), norm='forward')
...
class SpectralConv2d(nn.Module):
    ...
    def __init__(self, in_codim, out_codim, dim1, dim2, modes1=None, modes2=None, dropout_p=0):
        ...
        self.weights1 = nn.Parameter(torch.view_as_real(self.scale * torch.randn(in_codim, out_codim, self.modes1, self.modes2, dtype=torch.cfloat)))
        self.weights2 = nn.Parameter(torch.view_as_real(self.scale * torch.randn(in_codim, out_codim, self.modes1, self.modes2, dtype=torch.cfloat)))
    ...
    def forward(self, x, dim1=None, dim2=None):
        if dim1 is not None:
            ...
            self.dim2 = dim2
        batchsize = x.shape[0]
        x_ft = torch.fft.rfft2(x.to(torch.float32), norm='forward')
        out_ft = torch.zeros(batchsize, self.out_channels, self.dim1, self.dim2 // 2 + 1, device=x.device, dtype=torch.cfloat)
...
class FourierOperatorBlock2d(nn.Module):

    def __init__(self, in_codim, out_codim, dim1, dim2, modes1, modes2, Normalize=True, Non_Lin=True, dropout=0):
        super(FourierOperatorBlock2d, self).__init__()
        self.conv = SpectralConv2d(in_codim, out_codim, dim1, dim2, modes1, modes2)
        self.w = PointwiseConv2d(in_codim, out_codim, dim1, dim2)
        self.normalize = Normalize
        self.non_lin = Non_Lin
        if Normalize:
...
class SpectralConv3d_Uno(nn.Module):
    ...
    def forward(self, x, dim1=None, dim2=None, dim3=None):
        if dim1 is not None:
            ...
            self.dim3 = dim3
        batchsize = x.shape[0]
        x_ft = torch.fft.rfftn(x.to(torch.float32), dim=[-3, -2, -1], norm='forward')
...
class pointwise_op_3D(nn.Module):
    ...
    def forward(self, x, dim1=None, dim2=None, dim3=None):
        if dim1 is None:
            ...
            dim3 = self.dim3
        x_out = self.conv(x)
        ft = torch.fft.rfftn(x_out.to(torch.float32), dim=[-3, -2, -1])
